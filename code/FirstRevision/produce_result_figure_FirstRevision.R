...
for(s in 1:length(cond.mean)){
  ...
  #spo2/fio2 plotting only 010421
  dtc <- dtc %>% filter(biom == "biom1")
  p0 <- ggplot()+
  ...
  for(i in 2:length(cmean1)){ #s=2: currently at day s-1 = 1; i=13: project next 13 days
    ...
    dtc <- dtc.tmp %>% 
      ...
      separate(key, c('type', 'biom','inv'), "\\.") %>% 
      spread(type, cmean)
    # for spo2/fio2 plotting only 010421
    dtc <- dtc %>% filter(biom == "biom1")
    
    p <- p + 
      geom_line(data  = dtc, aes(x = interval, y = cmeank1, color = "Discharge"), alpha = 0.6, show.legend = F)+
      geom_point(data  = dtc %>% filter(interval == s-1+i), aes(x = interval, y = cmeank1, color = "Discharge", size = pprob1), alpha = 0.6, show.legend = F)+
      geom_line(data  = dtc, aes(x = interval, y = cmeank2, color = "Died"), alpha = 0.6, show.legend = F)+
      geom_point(data  = dtc %>% filter(interval ==  s-1+i), aes(x = interval, y = cmeank2, color = "Died", size = pprob2), alpha = 0.6, show.legend = F)+
      geom_line(data  = dtc, aes(x = interval, y = cmeank3, color = "Vent"), alpha = 0.6, show.legend = F)+
      geom_point(data  = dtc %>% filter(interval ==  s-1+i), aes(x = interval, y = cmeank3, color = "Vent", size = pprob3), alpha = 0.6, show.legend = F) +
      facet_grid(biom~., scales = "free_y", labeller = labeller(biom = biom.labeller))
  ...
  summ <- data.frame(type = c("Discharge", "Die", "Vent"),
                     sum.P = c(paste(round(100*sum(dtfull$pprob1, na.rm = T), 1), "%", sep=""),
                               paste(round(100*sum(dtfull$pprob2, na.rm = T), 1), "%", sep=""),
                               paste(round(100*sum(dtfull$pprob3, na.rm = T), 1), "%", sep="")))
  # summ <- data.frame(type = c("Discharge", "Die", "Vent"),
  #                    sum.P = c("12.0%",
  #                              paste(round(100*sum(dtfull$pprob2, na.rm = T), 1), "%", sep=""),
  #                              paste(round(100*sum(dtfull$pprob3, na.rm = T), 1), "%", sep="")))
  ...
  facet_bounds <- data.frame(ymin = c(90),
                             ymax = c(410),
                             breaks = c(50),
                             biom = c("biom1")) 
  summ.theme <- ttheme_minimal(core = list(bg_params = list(fill = c("#93DEEC","#F2918C","#C1E87D")),
                                           fg_params=list(cex=1.5)))
  
  pf <- p+
    theme_bw()+
    # scale_size_continuous(name=" ", range = c(0,10),breaks = seq(0,1,length.out=5), limits = c(0,1))+ 
    scale_size_continuous(name=" ", range = c(0,10),breaks = c(0.05,0.1, 0.15), limits = c(0,1))+ 
    scale_color_manual(
    ...
    annotation_custom(tableGrob(summ[,-1], theme = summ.theme, rows = NULL ,cols = NULL),
                      xmin=18, xmax = 20, ymin = 300, ymax = 400)
  p.all[[s]] <- pf
  #guides(size = F, color = F)
  

  # pt <- grid.arrange(tableGrob(summ[,-1], theme = summ.theme, rows = NULL ,cols = NULL))
  # 
  # ## original graph
  ...
  # my_table <- pt
  # 
  # plist <- list(my_hist + guides(size = F), my_table)
...
output_list <- list(p.all[[2]]+ 
                      theme(legend.box = "horizontal",
                            ...
                            plot.margin = margin(t=0,r=2,b=-0.5,l=0, "cm"),
                            axis.text.x = element_blank())+
                      guides(colour=guide_legend(override.aes=list(size = 4))), 
                    p.all[[4]]+guides(color = "none", size = "none")+
                      theme(plot.margin = margin(0,2,-0.6,0, "cm"),
                            axis.text.x = element_blank()), 
                    p.all[[6]]+guides(color = "none", size = "none")+
                      theme(plot.margin = margin(0,2,-0.6,0, "cm"),
                            axis.text.x = element_blank()), 
                    p.all[[8]]+guides(color = "none", size = "none")+
                      theme(plot.margin = margin(0,2,-0.6,0, "cm"),
                            axis.text.x = element_blank()), 
                    p.all[[10]]+guides(color = "none", size = "none")+
                      theme(plot.margin = margin(0,2,-0.6,0, "cm")))

plot <- plot_grid(plotlist = output_list, align="v", ncol = 1, nrow = length(output_list),
          rel_widths = c(1,1,1,1,1))
x.grob = textGrob("Day of Hospitalization",
                  gp = gpar(fontsize = fontsize))
y.grob = textGrob("SpO2/FiO2",
                  gp = gpar(fontsize = fontsize), rot = 90)
#pdf(paste0("paperPrep/finalizedCode/figs/result_figure_",sid,".pdf"), height = 18, width = 10)
pdf(paste0("paperPrep/finalizedCode/figs/result_figure2.pdf"), height = 14, width = 8)
