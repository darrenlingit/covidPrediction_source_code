...
load("paperPrep/finalizedCode/figs/calibration/forw_di=4_M.RData")
load("paperPrep/finalizedCode/figs/calibration/bkwr_di=8_M.RData")
load("paperPrep/finalizedCode/figs/calibration/forw_di=8_M.RData")
...
## calculate AUC of prosp and retrosp models for binary outcomes --------
i <- 8 #given data until day 2
...
f.prob.save <- forw.save8[[5]]
b.prob.save <- bkwr.save8[[5]]
...
save(auc_uet_8, file = paste0("paperPrep/finalizedCode/figs/calibration/",
                              "auc_di=",i,"_BinaryOutcome.RData"))

## calculate AUC of average of the the two methods -----
...
f.prob.save <- forw.save0[[5]]
b.prob.save <- bkwr.save0[[5]]
id_remove <- id_remove0
#id_remove <- NULL
datasurv_d0 <- list()
...
for(v in 1:5){
  ...
  for(s in 1:length(test_ids_4d)){
    ...
    if(sid %in% id_remove$osler_id){
      ...
      tmp.data.b <- rbind(tmp.data.b, data)
      #for prosp model
      data.f <- as.matrix(f.prob.save.cv[[v]][[s]])
      colnames(data.f) <- c('E1', 'E2', 'E3')
      data.f <- cbind(data.f, day = 0:20)
      data.f[is.na(data.f)] <- 0
      data.f <- as.data.frame(data.f) %>% mutate(cumE1 = cumsum(E1),
                                             cumE2 = cumsum(E2), 
                                             cumE3 = cumsum(E3))
      tmp.f <- dt1c %>% filter(osler_id == sid)
      endpoint.f <- min(unique(tmp.f$endpoint));W <- max(tmp.f$Wt)
      data.f <- data.f %>% mutate(O1 = ifelse(day==endpoint.f & W==1, 1, 0),
                              O2 = ifelse(day==endpoint.f & W==2, 1, 0),
                              O3 = ifelse(day==endpoint.f & W==3, 1, 0))
      data.f <- data.f %>% mutate(cumO1 = cumsum(O1),
                              cumO2 = cumsum(O2),
                              cumO3 = cumsum(O3))
      data.f$osler_id <- sid
      data.f$cv <- v
      tmp.data.f <- rbind(tmp.data.f, data.f)
    }
  }
  b.data.roc <- rbind(b.data.roc, tmp.data.b)
  f.data.roc <- rbind(f.data.roc, tmp.data.f)
...
datasurv.b<- datasurv.f <- datasurv.a <-  NULL
for(t in (i+1)){
  # get average of the two models
  datasurv.b <- b.data.roc %>% filter(day == t)
  datasurv.f <- f.data.roc %>% filter(day == t)
  
  # f.tmp.auc <- auc(datasurv.b$cumO2_new,datasurv.b$cumE2_new)
  # auc(datasurv.f$cumO2_new,datasurv.f$cumE2_new)
  
  if(isTRUE(all.equal(sum(datasurv.b$osler_id %in% datasurv.f$osler_id),
     sum(datasurv.f$osler_id %in% datasurv.b$osler_id),
     ncol(datasurv.b), ncol(datasurv.f)))){
    tmp.b <- datasurv.b %>% 
      dplyr::select(osler_id,cv, cumO1_new, cumO2_new,cumE1_new, cumE2_new) %>% 
      dplyr::rename(
                    cumE1_new_b = cumE1_new,
                    cumE2_new_b = cumE2_new)
    tmp.f <- datasurv.f %>% 
      dplyr::select(osler_id,cv,cumO1_new, cumO2_new,cumE1_new, cumE2_new) %>% 
      dplyr::rename(
                    cumE1_new_f = cumE1_new,
                    cumE2_new_f = cumE2_new)
    tmp <- tmp.b %>%  full_join(tmp.f, by = c("osler_id", "cv","cumO1_new", "cumO2_new"))
    datasurv.a <- tmp   
    datasurv.a$cumE1_new_ave = (datasurv.a$cumE1_new_b + datasurv.a$cumE1_new_f)/2
    datasurv.a$cumE2_new_ave = (datasurv.a$cumE2_new_b + datasurv.a$cumE2_new_f)/2
    }else{
    message("incorrect dimension")
  }
  
  datasurv.b$type <- "retrosp"
  datasurv.f$type <- "prosp"
  datasurv.a$type <- "average"
}
datasurv_d0 = list(datasurv.b = datasurv.b, 
                   datasurv.f= datasurv.f, 
                   datasurv.a = datasurv.a)

# save(datasurv_d0, file = paste0("paperPrep/finalizedCode/figs/calibration/",
#                          "datasurv_d",i,"_BinaryOutcome.RData"))

load("paperPrep/finalizedCode/figs/calibration/datasurv_d0_BinaryOutcome_day1Prediction.RData")
load("paperPrep/finalizedCode/figs/calibration/datasurv_d2_BinaryOutcome_day1Prediction.RData")
load("paperPrep/finalizedCode/figs/calibration/datasurv_d4_BinaryOutcome_day1Prediction.RData")
load("paperPrep/finalizedCode/figs/calibration/datasurv_d8_BinaryOutcome_day1Prediction.RData")

clean_datasurv <- function(datasurv_di){
  tmpb <- datasurv_di[[1]] %>% dplyr::select(osler_id, cv, type, day, cumO2_new, cumE2_new)
  tmpf <- datasurv_di[[2]]%>% dplyr::select(osler_id, cv, type, day, cumO2_new, cumE2_new)
  tmpa <- datasurv_di[[3]]%>% dplyr::select(osler_id, cv, type,  cumO2_new, cumE2_new_ave) %>% 
    dplyr::rename(cumE2_new = cumE2_new_ave) %>% mutate(day = unique(tmpb$day))
  out <- rbind(tmpb, tmpf, tmpa)
}

datasurv_d0_cleaned <- clean_datasurv(datasurv_d0)
datasurv_d2_cleaned <- clean_datasurv(datasurv_d2)
datasurv_d4_cleaned <- clean_datasurv(datasurv_d4)
datasurv_d8_cleaned <- clean_datasurv(datasurv_d8)

datasurv_all <- rbind(datasurv_d0_cleaned, datasurv_d2_cleaned, 
                      datasurv_d4_cleaned, datasurv_d8_cleaned)

auc.e2.ave <- auc(datasurv_all$cumO2_new[datasurv_all$type == "average"],
                  datasurv_all$cumE2_new[datasurv_all$type == "average"])
auc.e2.ave

auc.e2.pro <- auc(datasurv_all$cumO2_new[datasurv_all$type == "prosp"],
                  datasurv_all$cumE2_new[datasurv_all$type == "prosp"])
auc.e2.pro

auc.e2.retro <- auc(datasurv_all$cumO2_new[datasurv_all$type == "retrosp"],
                  datasurv_all$cumE2_new[datasurv_all$type == "retrosp"])
auc.e2.retro

### same as before but for day 20 auc (table1) ---------
i <- 8 #given data until day 2
## filter out patients have an event prior to day 4, keep only event occurred after day 4.
dt.endpoints <- dt.pre.mult.tmp %>% 
  dplyr::group_by(osler_id) %>% 
...
datasurv_d8 = list(datasurv.b = datasurv.b, 
                   datasurv.f= datasurv.f, 
                   datasurv.a = datasurv.a)

# save(datasurv_d8, file = paste0("paperPrep/finalizedCode/figs/calibration/",
#                          "datasurv_d",i,"_BinaryOutcome_day20Pred.RData"))

load("paperPrep/finalizedCode/figs/calibration/datasurv_d0_BinaryOutcome_day20Pred.RData")
load("paperPrep/finalizedCode/figs/calibration/datasurv_d2_BinaryOutcome_day20Pred.RData")
load("paperPrep/finalizedCode/figs/calibration/datasurv_d4_BinaryOutcome_day20Pred.RData")
load("paperPrep/finalizedCode/figs/calibration/datasurv_d8_BinaryOutcome_day20Pred.RData")
...
datasurv_d0_day20 <- clean_datasurv(datasurv_d0)
datasurv_d2_day20 <- clean_datasurv(datasurv_d2)
datasurv_d4_day20 <- clean_datasurv(datasurv_d4)
datasurv_d8_day20 <- clean_datasurv(datasurv_d8)

datasurv_day20 <- rbind(datasurv_d0_day20, datasurv_d2_day20, 
                      datasurv_d4_day20, datasurv_d8_day20)

gen_auc_table <- function(datasurv_d0_day20, datasurv_d2_day20,
                          datasurv_d4_day20, datasurv_d8_day20){
  auc_d0_pro <- auc(datasurv_d0_day20$cumO2_new[datasurv_d0_day20$type == "prosp"],
                    datasurv_d0_day20$cumE2_new[datasurv_d0_day20$type == "prosp"])
  auc_d2_pro <- auc(datasurv_d2_day20$cumO2_new[datasurv_d2_day20$type == "prosp"],
                    datasurv_d2_day20$cumE2_new[datasurv_d2_day20$type == "prosp"])
  auc_d4_pro <- auc(datasurv_d4_day20$cumO2_new[datasurv_d4_day20$type == "prosp"],
                    datasurv_d4_day20$cumE2_new[datasurv_d4_day20$type == "prosp"])
  auc_d8_pro <- auc(datasurv_d8_day20$cumO2_new[datasurv_d8_day20$type == "prosp"],
                    datasurv_d8_day20$cumE2_new[datasurv_d8_day20$type == "prosp"])
  
  auc_d0_retro <- auc(datasurv_d0_day20$cumO2_new[datasurv_d0_day20$type == "retrosp"],
                    datasurv_d0_day20$cumE2_new[datasurv_d0_day20$type == "retrosp"])
  auc_d2_retro <- auc(datasurv_d2_day20$cumO2_new[datasurv_d2_day20$type == "retrosp"],
                    datasurv_d2_day20$cumE2_new[datasurv_d2_day20$type == "retrosp"])
  auc_d4_retro <- auc(datasurv_d4_day20$cumO2_new[datasurv_d4_day20$type == "retrosp"],
                    datasurv_d4_day20$cumE2_new[datasurv_d4_day20$type == "retrosp"])
  auc_d8_retro <- auc(datasurv_d8_day20$cumO2_new[datasurv_d8_day20$type == "retrosp"],
                    datasurv_d8_day20$cumE2_new[datasurv_d8_day20$type == "retrosp"])
  
  auc_d0_ave <- auc(datasurv_d0_day20$cumO2_new[datasurv_d0_day20$type == "average"],
                      datasurv_d0_day20$cumE2_new[datasurv_d0_day20$type == "average"])
  auc_d2_ave <- auc(datasurv_d2_day20$cumO2_new[datasurv_d2_day20$type == "average"],
                      datasurv_d2_day20$cumE2_new[datasurv_d2_day20$type == "average"])
  auc_d4_ave <- auc(datasurv_d4_day20$cumO2_new[datasurv_d4_day20$type == "average"],
                      datasurv_d4_day20$cumE2_new[datasurv_d4_day20$type == "average"])
  auc_d8_ave <- auc(datasurv_d8_day20$cumO2_new[datasurv_d8_day20$type == "average"],
                      datasurv_d8_day20$cumE2_new[datasurv_d8_day20$type == "average"])
  out <- data.frame(prosp = c(auc_d0_pro, auc_d2_pro, auc_d4_pro, auc_d8_pro),
                    retrosp = c(auc_d0_retro, auc_d2_retro, auc_d4_retro, auc_d8_retro),
                    average = c(auc_d0_ave, auc_d2_ave, auc_d4_ave, auc_d8_ave)) 
  return(out)
}

gen_auc_table(datasurv_d0_day20, datasurv_d2_day20,
              datasurv_d4_day20, datasurv_d8_day20)
